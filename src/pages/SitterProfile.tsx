...
const SitterProfile: React.FC = () => {
  ...
  // Fetch sitter details using axios
  useEffect(() => {
    ...
    const fetchSitter = async () => {
      try {
        ...
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/api/sitters/${id}/`,
          {
            headers: {
        ...
        console.log("Fetched Sitter:", response.data);
  ...
  async function requestSitter() {
    ...
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_API_BASE_URL}/api/request-sitter/${sitter.id}/`,
        requestData,
        {
          headers: {
